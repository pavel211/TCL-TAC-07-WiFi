...
Предварительно можно исправить параметры в начале файла TCL_Conditioner.ino. Там задаются пароль доступа к интерфейсу обновления прошивки и подключения к точке доступа настройки устройства. По-умолчанию эти пароли admin и 12345678 соответственно.

Можно скачать и использовать скомпилированную прошивку TCL_Conditioner/TCL_Conditioner.ino.nodemcu.bin

При первом включении или при ошибке подклюения к WiFi устройсто запустится в режиме точки доступа с именем "TCL_AC_Setup" и паролем "12345678". Необходимо подключиться к ней и прописать параметры WiFi в течении 3 минут. Через 3 минуты будет выполнена перезагрузка и новая попытка подключения к WiFi.

Внешний вид web интерфейса:

![Alt text](web.png?raw=true "WEB интерфейс")

В меню Setup задаются параметры подключения к WiFi сети и MQTT брокеру (включая main_topic).
